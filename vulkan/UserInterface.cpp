...
void UserInterface::init(const VulkanData& renderData)
{
    vulkanData_ = renderData;
}
...
void UserInterface::preparePipeline(const VkPipelineCache pipelineCache, const VkRenderPass renderPass, const VkFormat colorFormat, const VkFormat depthFormat)
{
    // Pipeline layout
    ...
    VK_CHECK(vkCreatePipelineLayout(vulkanData_.device, &pipelineLayoutCreateInfo, nullptr, &pipelineLayout));
    ...
    VK_CHECK(vkCreateGraphicsPipelines(vulkanData_.device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline));
}

void UserInterface::prepareResources()
{
    ImGuiIO& io = ImGui::GetIO();
    ...
    size_t bufferSize = texWidth*texHeight * 4 * sizeof(char);
    ...
    fontImage = createImage(vulkanData_, imageInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, fontMemory );
    ...
    fontView = createImageView(vulkanData_.device, viewInfo);
    ...
    transitionImageLayout(vulkanData_, fontImage, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,  VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
    ...
    copyBufferToImage(vulkanData_, fontImage ,stagingBuffer, bufferCopyRegions);

       // Prepare for shader read

    transitionImageLayout(vulkanData_, fontImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,  VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    ...
    VK_CHECK(vkCreateDescriptorPool(vulkanData_.device, &descriptorPoolInfo, nullptr, &descriptorPool));
    ...
    VK_CHECK(vkCreateDescriptorSetLayout(vulkanData_.device, &descriptorLayout, nullptr, &descriptorSetLayout));

    // Descriptor set
    VkDescriptorSetAllocateInfo allocInfo = initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayout, 1);
    VK_CHECK(vkAllocateDescriptorSets(vulkanData_.device, &allocInfo, &descriptorSet));
    ...
    vkUpdateDescriptorSets(vulkanData_.device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
...
bool UserInterface::update()
{
    ...
    if ((vertexBuffer.buffer == VK_NULL_HANDLE) || (vertexCount != imDrawData->TotalVtxCount)) {
        vertexBuffer.create(vulkanData_, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, vertexBufferSize);
    ...
    if ((indexBuffer.buffer == VK_NULL_HANDLE) || (indexCount < imDrawData->TotalIdxCount)) {
        indexBuffer.create(vulkanData_, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, indexBufferSize);
...
void UserInterface::draw(const VkCommandBuffer commandBuffer)
{
    ImDrawData* imDrawData = ImGui::GetDrawData();
...
bool UserInterface::checkBox(const char *caption, bool *value)
{
...
bool UserInterface::checkBox(const char *caption, int32_t *value)
{
    bool val = (*value == 1);
...
bool UserInterface::radioButton(const char *caption, bool value)
{
...
bool UserInterface::inputFloat(const char *caption, float *value, float step, uint32_t precision)
{
...
bool UserInterface::sliderFloat(const char *caption, float *value, float min, float max)
{
...
bool UserInterface::sliderInt(const char *caption, int32_t *value, int32_t min, int32_t max)
{
...
bool UserInterface::comboBox(const char *caption, int32_t *itemindex, std::vector<std::string> items)
{
    if (items.empty()) {
        return false;
    }
...
bool UserInterface::colorPicker(const char *caption, float *color)
{
...
void UserInterface::cleanUp()
{
    ...
    indexBuffer.cleanUp();
    vkDestroyImageView(vulkanData_.device, fontView, nullptr);
    vkDestroyImage(vulkanData_.device, fontImage, nullptr);
    vkFreeMemory(vulkanData_.device, fontMemory, nullptr);
    vkDestroySampler(vulkanData_.device, sampler, nullptr);
    vkDestroyDescriptorSetLayout(vulkanData_.device, descriptorSetLayout, nullptr);
    vkDestroyDescriptorPool(vulkanData_.device, descriptorPool, nullptr);
    vkDestroyPipelineLayout(vulkanData_.device, pipelineLayout, nullptr);
    vkDestroyPipeline(vulkanData_.device, pipeline, nullptr);
}
